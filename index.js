    ...
    interval = setInterval(() => {
      try {
        ...
        devices.forEach(device => {
          const value = readAndConvertValue(bus, device);
          logDebug(`Device read, name: ${device.name}, value: ${value}`);
          updateSingnalK(app, device, value);
          status = status + device.name + ': ' + value + device.signalk.units + ', ';
        });
...
function readAndConvertValue(bus, device) {
...
function updateSingnalK(app, device, value) {
